...
class CategoryCell: UITableViewCell {
    ...
    func configureCell(genre: String, books: [Book]) {
        if let url1 = URL(string: "https" + books[0].smallThumbnail.dropFirst(4)) {
            categoryImg1.kf.setImage(with: url1)
        }
        
        if let url2 = URL(string: "https" + books[1].smallThumbnail.dropFirst(4)) {
            categoryImg2.kf.setImage(with: url2)
        }
        
        if let url3 = URL(string: "https" + books[2].smallThumbnail.dropFirst(4)) {
